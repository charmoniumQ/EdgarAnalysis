...
def retrieve_features_for_company(name,year):
    ...
    conn = connect_to_db()
    sql = "SELECT name, year, quarter, improvement, anger, disgust, fear, joy, sadness, sentiment, sentiment_type, ticker FROM features WHERE LOWER(REPLACE(REPLACE(REPLACE(CONCAT('%',name,'%'),' ','%'),'\\'',''),',','')) LIKE LOWER(REPLACE(REPLACE(CONCAT('%','{name}','%'),' ','%'),',','')) AND year = {year} AND ticker = '{ticker}' AND UseBit = 1".format(name=name,year=str(year),ticker=ticker)
    cur = conn.cursor()
